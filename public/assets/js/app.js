...
app.controller("solicitar_ticket_controller", function($scope, $http, $window) {
    ...
    $scope.listas = {};
    $scope.usuarios = {};
    $scope.nueva_atencion={};
    $scope.nueva_atencion.telefono="";
    ...
    $scope.registrar_atencion_sms = function() {
        ...
        if ($('.has-error').length) {
            ...
            console.log($scope.nueva_atencion.telefono);
            if ($scope.nueva_atencion.telefono == "") {
                error('es necesario un numero de telefono para esta opción');
            }
            else {
                $scope.nueva_atencion.user_id = $scope.sesion.id;
